...
public class Game extends Application implements GameScreen {
        ...
        for(int y = 0; y < this.height; ++y) {
            for(int x = 0; x < this.width; ++x) {
                ...
                if (this.showGrid && children.size() > 0) {
                    ...
                    cell.setStroke(this.toFXColor(src.Color.BLACK));
                }
    ...
    private void createScorePanel() {
        ...
        this.scoreText.setFill(this.toFXColor(src.Color.BLACK));
        StackPane scorePane = new StackPane(new Node[]{this.scoreText});
        scorePane.setBorder(new Border(new BorderStroke[]{new BorderStroke(this.toFXColor(src.Color.BLACK), BorderStrokeStyle.SOLID, CornerRadii.EMPTY, BorderWidths.DEFAULT)}));
        scorePane.setLayoutY((double)(this.height * cellSize + 110 + 6));
        int scoreHeight = 20;
        Rectangle rectangle;
        if (this.showGrid) {
            rectangle = new Rectangle((double)((this.width * cellSize - 1) / 2), (double)scoreHeight, this.toFXColor(src.Color.WHITE));
            scorePane.setLayoutX((double)(125 + (this.width * cellSize - 1) / 4));
        } else {
            rectangle = new Rectangle((double)(this.width * cellSize / 2), (double)scoreHeight, this.toFXColor(src.Color.WHITE));
    ...
    public void setCellColor(int x, int y, src.Color color) {
        if (color != null && color != src.Color.NONE) {
            ObservableList<Node> children = this.cells[y][x].getChildren();
    ...
    public void setCellTextColor(int x, int y, src.Color color) {
        ObservableList<Node> children = this.cells[y][x].getChildren();
        if (children.size() > 1) {
            Text text = (Text)children.get(1);
    ...
    public src.Color getCellTextColor(int x, int y) {
        ObservableList<Node> children = this.cells[y][x].getChildren();
        if (children.size() > 1) {
            Text text = (Text)children.get(1);
    ...
    private src.Color fromFXColor(Color color) {
        ...
        src.Color[] result = new src.Color[]{src.Color.NONE};
        optionalColorEntry.ifPresent((entry) -> {
            result[0] = src.Color.valueOf((String)entry.getKey());
        });
        return result[0];
    }

    private Color toFXColor(src.Color color) {
        if (color == src.Color.NONE) {
    ...
    public void setCellValueEx(int x, int y, src.Color cellColor, String value) {
        this.setCellValue(x, y, value);
        this.setCellColor(x, y, cellColor);
    }

    public void setCellValueEx(int x, int y, src.Color cellColor, String value, src.Color textColor) {
        this.setCellValueEx(x, y, cellColor, value);
        this.setCellTextColor(x, y, textColor);
    }

    public void setCellValueEx(int x, int y, src.Color cellColor, String value, src.Color textColor, int textSize) {
        this.setCellValueEx(x, y, cellColor, value, textColor);
        this.setCellTextSize(x, y, textSize);
    }

    public void showMessageDialog(src.Color cellColor, String message, src.Color textColor, int textSize) {
    ...
    public void setLives(int lives) {
        if (this.livesText == null) {
            ...
            this.livesText.setFill(this.toFXColor(src.Color.ORANGE));
